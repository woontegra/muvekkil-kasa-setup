...
function applyMax(formatted: string, maxValue: number | undefined): string {
  ...
  // Boş / ara yazım (yalnızca virgül vb.) üst sınıra zorlanmaz — input kilitlenmez
 ...
 *
 * Odak varken değer iç draft'ta tutulur; parent re-render'ı yazmayı ezmez.
 * Boş string korunur (otomatik 0,00'a dönülmez). Caret tek seferlik düzeltilir.
 */
export function MoneyInput({
  ...
  // Odak yokken parent value → draft senkronu. Odak varken parent ezmesin.
  useEffect(() => {
  ...
  function commitTyping(raw: string, caretInRaw: number) {
    ...
    // Tek seferlik caret düzeltmesi (zincir / sürekli rAF yok)
  ...
  function handleBlur(_e: FocusEvent<HTMLInputElement>) {
    ...
    if (n == null) {
      // Geçersiz ara metin: boşaltma; kullanıcı tekrar odaklanıp düzeltebilir
      return;
    }
    if (!allowZero && n <= 0) {
      setDraft("");
  ...
  function handleFocus(e: FocusEvent<HTMLInputElement>) {
    ...
    setDraft(value);
    // Tam formatlanmış tutarda (blur sonrası) seç; boş alanda select yapma —
    // sil-yaz döngüsünde caret/selection kilitlenmesini önler.
