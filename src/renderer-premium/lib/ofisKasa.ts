...
import { formatSignedTry, formatTry, parseCurrencyInputTR } from "./format";

export { formatSignedTry, hesaplaOfisKasaDuzeltme };
...
export function ofisKasaSatirSinifi(h: OfisKasaHareketListeSatir): string {
  ...
  if (h.duzeltmeMi || h.islemTipi === "DUZELTME") {
    parcalar.push("pm-ofis-row--duzeltme");
  } else if (h.hasCorrection) {
    parcalar.push("pm-ofis-row--corrected");
  }
...
export function onayBadgeTone(h: OfisKasaHareketListeSatir): "warning" | "success" | "info" {
  if (h.onayDurumu === "ONAYSIZ") return "warning";
  if (h.otomatikOnayMi) return "info";
  return "success";
}
...
export function hareketGelirGiderHucre(h: OfisKasaHareketListeSatir): { gelir: number | null; gider: number | null } {
  if (h.islemTipi === "GELIR") return { gelir: h.tutar, gider: null };
  if (h.islemTipi === "GIDER") return { gelir: null, gider: h.tutar };
  const etki = duzeltmeKasaEtkisiFromRow(h);
  if (etki > 0) return { gelir: etki, gider: null };
  if (etki < 0) return { gelir: null, gider: Math.abs(etki) };
  return { gelir: null, gider: null };
}
