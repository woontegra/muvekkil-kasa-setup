...
export function LicenseTrialSetupPage() {
  const { setupFirst } = usePremiumAuth();
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!trial.ok) {
        setError(
          trial.code === "TRIAL_ALREADY_USED" ? TRIAL_ALREADY_USED_MESSAGE : (trial.error ?? "Deneme başlatılamadı."),
        );
        return;
      }
  ...
  return (
    <PremiumAuthLayout variant="setup">
      <div className="pm-auth-card pm-auth-card--enter pm-auth-card--wide">
        <h2 className="pm-auth-card-title">7 Gün Ücretsiz Dene</h2>
        <p className="pm-auth-card-sub">Deneme lisansı ve yönetici hesabınızı oluşturun.</p>
        ...
        <form className="pm-auth-form" onSubmit={(e) => void onSubmit(e)}>
          <PremiumFormField label="Ad soyad" htmlFor={idAd}>
            <PremiumTextInput id={idAd} autoComplete="name" value={adSoyad} onChange={(e) => setAdSoyad(e.target.value)} disabled={busy} />
          </PremiumFormField>
          <PremiumFormField label="E-posta" htmlFor={idEmail}>
            <PremiumTextInput id={idEmail} autoComplete="email" value={eposta} onChange={(e) => setEposta(e.target.value)} disabled={busy} placeholder="ornek@mail.com" />
          </PremiumFormField>
          <PremiumFormField label="Telefon" htmlFor={idPhone}>
            <PremiumTextInput id={idPhone} autoComplete="tel" value={telefon} onChange={(e) => setTelefon(e.target.value)} disabled={busy} placeholder="05xx xxx xx xx" />
          </PremiumFormField>
          ...
          <PremiumFormField label="Şifre tekrar" htmlFor={idPass2}>
            <PremiumTextInput id={idPass2} type={showPass ? "text" : "password"} autoComplete="new-password" value={sifre2} onChange={(e) => setSifre2(e.target.value)} disabled={busy} />
          </PremiumFormField>
          </div>
          <PremiumFormField label="Güvenlik sorusu" htmlFor={idGuvenlikSoru}>
            <select id={idGuvenlikSoru} className="pm-input pm-select" value={guvenlikSorusuKodu} onChange={(e) => setGuvenlikSorusuKodu(e.target.value)} disabled={busy}>
          ...
          <PremiumFormField label="Güvenlik cevabı" htmlFor={idGuvenlikCevap}>
            <PremiumTextInput id={idGuvenlikCevap} value={guvenlikCevabi} onChange={(e) => setGuvenlikCevabi(e.target.value)} disabled={busy} />
          </PremiumFormField>
          <PremiumButton type="submit" className="pm-auth-submit" disabled={busy}>
            {busy ? "Hazırlanıyor…" : "Denemeyi Başlat"}
          </PremiumButton>
          <Link to="/lisans" className="pm-btn pm-btn--ghost" style={{ textAlign: "center" }}>
            Geri
          </Link>
        </form>
      </div>
