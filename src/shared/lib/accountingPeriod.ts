import type { AccountingPeriod, AccountingPeriodMode } from "../types/accountingPeriod";
...
/** Yerel takvim günü YYYY-MM-DD (TR / cihaz yerel saati). */
...
export function isAccountingPeriodMode(v: unknown): v is AccountingPeriodMode {
...
export function getCurrentAccountingPeriod(mode: AccountingPeriodMode, date: Date | string = new Date()): AccountingPeriod {
  return getAccountingPeriod(mode, date);
}
...
export function getNextAccountingPeriod(period: AccountingPeriod): AccountingPeriod | null {
  const current = getCurrentAccountingPeriod(period.mode);
  const p = parseYmd(period.bas);
  if (!p) return null;
...
export function isCurrentAccountingPeriod(period: AccountingPeriod, now: Date | string = new Date()): boolean {
  const cur = getCurrentAccountingPeriod(period.mode, now);
  return period.bas === cur.bas && period.bit === cur.bit;
}

/** Aktif (veya referans tarihli) hesap döneminin bas/bit aralığı. */
export function getActiveAccountingPeriodRange(
  mode: AccountingPeriodMode,
  referenceDate: Date | string = new Date(),
): { bas: string; bit: string; etiket: string; period: AccountingPeriod } {
  const period = getAccountingPeriod(mode, referenceDate);
  return { bas: period.bas, bit: period.bit, etiket: period.etiket, period };
}
